...
public struct Profile: Codable {
    ...
    ///
    ///
    ///- Nel _Bot Telegram_, nel _Web Portal_ e tramite richieste per riga di comando, come ad esempio _Postman_, il codice di sicurezza verrà generato dal Server sotto forma di una serie di 24 caratteri alfanumerici (a simboli esclusi), ad esempio `tgbGC9yzT8zwNaItXWJtorqm`.  I primi tre caratteri compongono un codice di riferimento che identifica l'ambiente della sessione. Scopri di più nella sezione <doc:Server###Codici-di-Riferimeto-per-gli-Ambienti>.
    ///
    ...
    init(username: String, secureCode: String, sessionID: String = UUID().uuidString, activeSessions: [String]) {
